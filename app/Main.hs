...
parseSpecialFile ignoreFile@".pankyignore" = modify (\s -> s {sfPankyIgnoreFile = Just ignoreFile})
...
constructDeckTree' path prefList = do
  ...
  let (_, specialFileInfo) = runState (parseSpecialFiles specialFiles) def
      deckName = fromMaybe (takeBasePathName path) $ sfDeckNameFile specialFileInfo
      prefix' = prefList ++ [T.pack deckName]
  filesToIgnore <- case sfPankyIgnoreFile specialFileInfo of
    Just fn -> LTO.readFile (path </> fn) >>= (return . T.lines)
    Nothing -> pure []
